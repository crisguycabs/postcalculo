...
class Ui_MainProtoV2(object):
    ...
    def EditarColumnaParam(self):
        ...
        if (self.ruta_modelo == ""):
            self.statusBar.showMessage(
                "No se pueden crear/modificar columnas hasta que no se escoja un modelo abstracto")
        ...
        if (current["TIPO_DATO_GAMS"] == "QUERY"):
            ...
            if (self.dimensiones):
                if (type(self.dimensiones) == list):
                    self.uiQuery.lblDimensiones.setText(
                        "Existe un conjunto previo de dimensiones: " + ",".join(self.dimensiones))
                else:
            ...
            if (myColParamChild.exec() == QtWidgets.QDialog.Accepted):
                ...
                if (self.uiParam.txtNombreColumna.text()):
                    ...
                    self.error_dialog.showMessage(
                        "No se incluyó un nombre de la columna. Se cancela la creación de la nueva columna")
                    return
                ...
                for i in range(self.uiParam.tableWidget.rowCount()):
                    ...
                    if item.checkState() == QtCore.Qt.Checked:
                        ...
                        lista.append((self.uiParam.tableWidget.item(i, 1).text(),
                                      self.uiParam.tableWidget.item(i, 2).text(), temp.iloc[0, 0]))
                print(lista)
                        ...
                        for i in range(1, len(lista)):
                            for j in range(0, len(lista) - 1):
                                if (lista[i][1] != lista[j][1]):
                                    self.statusBar.showMessage(
                                        "Las dimensiones seleccionadas no coinciden. Modifique su selección")
                                    self.error_dialog.showMessage(
                                        "Las dimensiones seleccionadas no coinciden. Modifique su selección")
                                    return
    ...
    def SelModelo(self):
        ...
        file_path1 = filedialog.askopenfilename(title="Seleccionando modelo abstracto...",
    ...
    def NuevaColumnaQuery(self):
        ...
        if (myColQueryChild.exec() == QtWidgets.QDialog.Accepted):
            ...
            if (self.uiQuery.txtDimensiones.text()):
                if (self.dimensiones):
                    ...
                    if (self.dimensiones != self.uiQuery.txtDimensiones.text()):
                        ...
                        self.error_dialog.showMessage(
                            "Las dimensiones incluidas no coinciden con las previas. Se cancela la creación de la nueva columna")
                        return
                else:
                ...
                self.error_dialog.showMessage(
                    "No se incluyeron dimensiones en la columna. Se cancela la creación de la nueva columna")
                return
            ...
            if (self.uiQuery.txtSql.toPlainText()):
                ...
                self.error_dialog.showMessage(
                    "No se incluyó una instrucción SQL. Se cancela la creación de la nueva columna")
                return
        ...
        if (myColParamChild.exec() == QtWidgets.QDialog.Accepted):
            ...
            if (self.uiParam.txtNombreColumna.text()):
                ...
                for i in range(self.ncol):
                    ...
                    if (self.sabana[tempnom][0]["NOMBRE_COLUMNA"] == self.uiParam.txtNombreColumna.text()):
                        ...
                        self.error_dialog.showMessage(
                            "Ya existe una columna con el nombre " + self.uiParam.txtNombreColumna.text() + ". Se cancela la creación de la nueva columna")
                        return None
            ...
            for i in range(self.uiParam.tableWidget.rowCount()):
                ...
                if item.checkState() == QtCore.Qt.Checked:
                    ...
                    temp = self.data.loc[self.data["nombre_dato"] == self.uiParam.tableWidget.item(i, 1).text()]
                    lista.append((
                                 self.uiParam.tableWidget.item(i, 1).text(), self.uiParam.tableWidget.item(i, 2).text(),
                                 temp.iloc[0, 0]))
            print(lista)
